...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        exp, str
    ):  # CHANGE THIS CONDITION FOR Q4 where are variables stored?
...
def eval_and(expressions):
    ...
    while (not res is scheme_f) and isinstance(next, Pair):
        res = calc_eval(next.first)
        next = next.rest
    return res
...
def eval_define(expressions):
    ...
    2
    """
    assert isinstance(expressions, Pair)
